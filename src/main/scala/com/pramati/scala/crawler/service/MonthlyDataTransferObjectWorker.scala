...
class MonthlyDataTransferObjectWorker(monthlyDataBean: MonthlyDataTransferObject) extends Callable[List[MailArchiveDataTransferObject]]{
  ...
  private def processMontlyDataBean(bean: MonthlyDataTransferObject): List[MailArchiveDataTransferObject] = {
    ...
    // it might contain more than 1 page
  ...
  private def go (pageNumber: Int,bean: MonthlyDataTransferObject) : List[MailArchiveDataTransferObject] = {
    if(pageNumber < 0) List.empty
    else {
      val url = s"${WebCrawlerProperties.getURL}${bean.href}?${pageNumber.toString}"
      WebCrawlerParsingUtils.parseArchivesMailsPage(WebCrawlerUtils.readDataFromURL(url), bean) ::: go (pageNumber -1,bean)
    }
  }
}
